...
def load_data(filepath):
    ...
    # 分离特征与标签
    y = df.pop('label')
    X = df[["login", "name", "email", "bio", "Number of followers", "Number of following", "tfidf_similarity", "Number of Activity",
...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    # 计算 AUC值
    auc_data = roc_auc_score(y_test, y_pred_proba)
...
def train_and_evaluate_model(model_list, X_train, X_test, y_train, y_test):
    ...
    # 使用pandas的DataFrame构造函数将模型在测试集上的评估指标结果转换成DataFrame
